  ...
  return (
    <div className="inline-flex flex-col items-start gap-1">
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-300 transition-colors hover:bg-slate-700 hover:text-purple-400 disabled:cursor-not-allowed disabled:opacity-50"
      >
        {loading ? (
          <div className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-slate-600 border-t-purple-400" />
        ) : (
      ...
      {provider && (
        <p className="text-[10px] text-purple-500/70">via {provider}</p>
      )}
      {error && (
        <div className="max-w-sm rounded-md bg-red-950/50 border border-red-900/50 p-2">
          ...
          {error.includes("aistudio.google.com") && (
            <a
              ...
              className="mt-1 inline-block text-[11px] text-purple-400 underline hover:text-purple-300"
            >
